...
var handleSearchResults = function(data) {
	...
	if (json.hits) {
		...
		$.each(json.hits,function(index, hit) {
			...
			*/
			...
			link = '<a target="_blank" href="'+title+''+'' +'">' +icon+ title+'</a>';
			if (hit.highlights) {
				// add highlight
	...
	var lastpage;
	if(Math.ceil(to/searchSize)+9>nbPage) {
		lastpage=nbPage;
	} else {
		lastpage=Math.ceil(to/searchSize)+9;
	}
	
	
	for (var i=Math.ceil(from/searchSize)+1; i<=lastpage; i++) {
